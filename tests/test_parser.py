...
class MetabaseParserTests(TestCase):
    ...
    def test_register_objects(self):
        ...
        conf = MetabaseParser()
        self.assertEqual(0, len(conf.groups.keys()))

        conf.register_objects(objects, "groups")

        self.assertEqual(2, len(conf.groups.keys()))
        self.assertIsInstance(conf.groups["Administrator"], Group)
        self.assertIsInstance(conf.groups["Developers"], Group)
    ...
    def test_register_object(self):
        ...
        self.assertEqual(conf.users["foo"], user)
